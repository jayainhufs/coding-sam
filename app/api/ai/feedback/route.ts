...
const BodySchema = z.object({
  step: StepEnum,
  // userInput 또는 promptOverride 중 하나는 필수
  ...
  (data) => data.userInput || data.promptOverride,
  { message: 'userInput 또는 promptOverride 중 하나는 필수입니다.' }
)
...
function buildPrompt(step: z.infer<typeof StepEnum>, mode: 'hint' | 'code-suggest' | undefined, userInput: string, problem: {title?: string; description?: string}) {
  const base = `
당신은 코딩 학습용 AI 튜터입니다. 우리는 코딩 방식을 총 5가지로 나눌거에요. 문제 이해, 문제 분해, 문제 패턴 인식, 문제 추상화하기, 의사코드 설계입니다.
...
[추가 요구] 지금까지의 피드백과 학생의 내용을 바탕으로 "짧은 스니펫" 또는 "의사코드"를 10~20줄 내로 제안하세요.`
  }
  return base
}
...
export async function POST(req: Request) {
  try {
    ...
    // promptOverride가 있으면 그대로 사용, 없으면 buildPrompt로 생성
    const prompt = promptOverride || buildPrompt(step, mode, userInput || '', problem)
